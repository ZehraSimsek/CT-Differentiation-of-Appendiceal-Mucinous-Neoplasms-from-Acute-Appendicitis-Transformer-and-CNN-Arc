...
def load_run_aucs(folder):
    ...
    for run_idx in [1, 2, 3]:
        p = BASE / folder / f"run_{run_idx:02d}" / "external_test" / "_external_test_metrics.csv"
        if not p.exists():
            aucs.append(np.nan)
            continue
        try:
...
def main():
    print("===  MASTER EVALUATION BAŞLIYOR ===")
    run_details = []
    metric_rows = []
    y_true_ref = None
    ...
    run_df.to_csv(BASE / "_MASTER_run_details.csv", index=False)
    print("\n" + "="*70)
    ...
    metrics_df.to_csv(BASE / "_MASTER_full_metrics.csv", index=False)
    print("\n" + "="*70)
    ...
    print("  - _MASTER_run_details.csv")
    print("  - _MASTER_full_metrics.csv")
