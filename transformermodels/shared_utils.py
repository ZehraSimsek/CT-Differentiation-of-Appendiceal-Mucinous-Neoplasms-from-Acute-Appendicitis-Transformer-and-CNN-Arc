...
def pos_weight_from_labels(labels):
    ...
    şeklinde overfit olmasını kırmak için pos_weight=1.5 olarak dengelendi.
    """
    return 1.5
...
def print_full_metrics_table(metrics, ci, model_name, threshold_name):
    ...
    rows = [
        ...
        ("Sensitivity (Recall)", "sens", metrics['sensitivity']),
        ("Specificity",  "spec", metrics['specificity']),
        ("PPV (Precision)", "ppv",  metrics['ppv']),
        ("NPV",          "npv",  metrics['npv']),
...
def train_one_epoch(model, loader, optimizer, criterion, device, mixup_alpha=0.0, accum_steps=1):
    ...
    (küçük dataset + küçük batch_size'dan gelen gürültülü gradyanı azaltmak için).
    """
    model.train()
    total_loss = 0
...
def evaluate_external_test_ensemble(model_builder, base_dir, config, test_csv_path,
    ...
    disp_cols = ["fold", "auc_roc", "auc_pr", "sensitivity", "specificity", "accuracy", "f1", "ppv", "npv",
                 "tp", "fp", "fn", "tn"]
