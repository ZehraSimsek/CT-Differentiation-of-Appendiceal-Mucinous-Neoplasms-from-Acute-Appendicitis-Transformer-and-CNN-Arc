...
Strateji: SwinUNETR backbone TAMAMEN dondurulmuş (pretrained weights korunur).
Sadece küçük head eğitiliyor: ~50K parametre.
Tıbbi veri setlerindeki (165 hasta) overfitting riskini sıfıra indirmek ve 
dış test (external test) setlerinde en yüksek klinik doğruluğu elde etmek için 
Linear Probing ve SWA teknikleri ile SOTA performans hedeflenmektedir.
...
Çıktılar:
    ...
    experiments/swinunetr_lp/external_test/q1_external_test_metrics.csv
...
CONFIG.update({
    ...
    "patience":          25,
    "swa_start":         50,
    "lr":                5e-4,   # Backbone donduruldu → head hızlı öğrenebilir
    "weight_decay":      1e-2,
    "warmup_epochs":     5,
    "min_sens_floor":    0.85,
    "min_spec_floor":    0.60,
...
def run_one_fold(train_df, val_df, fold_idx, config, output_dir):
    ...
    for epoch in range(1, config["n_epochs"] + 1):
        ...
        # Böylece kısıt sağlayan GÜÇSÜZ bir model bile, kısıt sağlamayan GÜÇLÜ bir modeli geçer.
        # Hiçbir model kısıt sağlayamazsa (Fold 2 gibi), kendi aralarında en iyi composite olanı (fallback) kaydeder.
        ...
        if score_for_selection > best_score:
            ...
            patience_cnt += 1
            if epoch >= swa_start and patience_cnt >= config["patience"]:
    ...
    if epoch >= swa_start:
        ...
        swa_c = clinical_composite(
            tp_/(tp_+fn_+1e-9), auc_s, tn_/(tn_+fp_+1e-9),
            f1=float(_f1(y_t, yp_s, zero_division=0)))
        print(f"  SWA composite: {swa_c:.4f} | Best regular: {best_score:.4f}")
        if swa_c > best_score:
            torch.save({"model_state_dict": swa_model.module.state_dict(),
                        "val_auc": auc_s, "composite": swa_c,
                        "epoch": epoch}, fold_dir / "best_model.pt")
