...
CONFIG.update({
    ...
    "lr":                2e-4,
    "weight_decay":      1e-2,
    "mae_mask_ratio":    0.60,      # Daha az maske → daha zengin anatomik temsil
    "mae_epochs":        80,
...
class TransformerBlock(nn.Module):
    def __init__(self, dim=192, num_heads=6, mlp_ratio=4.0, dropout=0.2):
        super().__init__()
...
def main():
    ...
    # MAE pretraining için TÜM veri (train+val+test birleşik manifest, etiketsiz kullanılıyor)
    all_csvs = list(DATAS_DIR.glob("fold_*_train.csv")) + list(DATAS_DIR.glob("fold_*_val.csv")) + [test_csv_path]
    manifest_df = pd.concat([pd.read_csv(c) for c in all_csvs], ignore_index=True).drop_duplicates("patient_id")
    print(f"MAE pretraining manifest: {len(manifest_df)} benzersiz hasta")
