...
Bu model, tıbbi 3D segmentasyon/sınıflandırma görevlerinde state-of-the-art (SOTA)
performans elde etmek amacıyla özel olarak optimize edilmiştir. 
Multi-Scale Feature Fusion ve CBAM-3D mekanizmalarıyla zenginleştirilmiş olup, 
güçlü regülarizasyon teknikleri (MixUp, SWA, ClinicalFocalLoss) ile 
küçük veri setlerindeki (165 hasta) aşırı öğrenme (overfitting) riski kırılmıştır.
...
Çıktılar:
    ...
    experiments/segformer3d/external_test/q1_external_test_metrics.csv
    experiments/segformer3d/train_log.txt
"""
import sys, os
sys.path.insert(0, os.path.abspath("."))
from shared_utils import *  # noqa: F401,F403  (torch, nn, F, np, pd, Path, DataLoader, vb. dahil)
...
CONFIG["mixup_alpha"] = 0.1      # Hafif Mixup
CONFIG["weight_decay"] = 5e-3    # Aşırı regülarizasyonu azalt
CONFIG["focal_gamma"] = 1.0
CONFIG["label_smoothing"] = 0.05


# ============================================================
# Model: SegFormer3D-MSCA
# 1. Multi-Scale Feature Fusion (Stage3 + Stage4 birleşimi)
# 2. CBAM-3D: en derin özniteliklerde spatial+channel attention
# 3. Overlap patch embedding + efficient (SR-ratio'lu) self-attention
# ============================================================
...
class SegFormer3DClassifier(nn.Module):
    def __init__(self, in_channels=1, num_classes=2):
        super().__init__()
        # Q1 upgrade: daha geniş temsil kapasitesi + derinleşen son aşamalar
...
# ============================================================
# Q1 UPGRADE: SegFormer3D MAE Pretraining
# MAE-TinyTransformer'ın Q1 basan stratejisinin aynısı:
# Encoder'a "maskeli yama rekonstrüksiyonu" görevi verip
# 241 hasta ile self-supervised öğretiyoruz. Ardından
# fine-tuning aşamasında bu ağırlıkları SegFormer'a aktarıyoruz.
# Sonuç: from-scratch yerine pretrained → Q1 metric hedeflenebilir.
# ============================================================
class SegFormer3DMAE(nn.Module):
    ...
    def __init__(self, in_channels=1, mask_ratio=0.75):
        super().__init__()
        # MAE encoder mimarisi fine-tune modeliyle birebir aynı olmalı
        embed_dims = [32, 64, 128, 256]
        num_heads  = [2, 4, 8, 16]
        sr_ratios  = [4, 2, 2, 1]
        depths = [2, 2, 3, 3]
        ...
        # Basit Decoder: son feature map'i orijinal input boyutuna upscale et
    ...
    def forward(self, x):
        B, C, D, H, W = x.shape
        # MAE masking: rastgele patch'leri sıfırla
        ...
        # Loss: sadece maskeli bölgelerde MSE
...
def pretrain_segformer_mae(config, device, save_path):
    ...
    # Tüm hastaları etiket fark etmeksizin kullan
    ...
    # Sadece encoder ağırlıklarını kaydet
    encoder_sd = {
...
def run_one_fold_segformer3d(train_df, val_df, fold_idx, config, output_dir):
    ...
    # Q1: MAE pretrained encoder ağırlıklarını yükle (varsa)
    ...
    # Q1: Loss için standart dinamik weighting (1.5x FN penalty)
    ...
    # Q1: Layer-wise Learning Rate (Pretrained backbone'u bozmamak için)
    ...
    # SWA (Stochastic Weight Averaging) — diğer iki mimaride (Attention-SwinUNETR,
    # MAE-TinyTransformer) var, burada eksikti. Tek "şanslı" epoch'un ağırlıklarını
    # dondurmak yerine son %30 epoch'un ağırlık ortalamasını alır — küçük val fold'daki
    # (~40 hasta) gürültüye karşı daha kararlı bir final model verir (fold-bazlı metrik
    # ile aggregate OOF arasındaki farkın büyük kısmı bu gürültüden kaynaklanıyordu).
    ...
    for epoch in range(1, config["n_epochs"] + 1):
        ...
        # Sıkı Q1 Çift Kısıt (Dual Constraint) Eşik Değerini Kullan (shared_utils.py)
    ...
    if epoch >= swa_start:
        ...
        torch.optim.swa_utils.update_bn(_image_generator(), swa_model, device=DEVICE)
        ...
        if swa_composite >= best_score * 0.98:
            ...
            print("  SWA model kaydedildi (genelleme için tercih edildi, tek-epoch gürültüsüne karşı daha kararlı).")
...
def main():
    ...
    # ── Q1 STAGE 1: Self-Supervised MAE Pretraining ──────────────────────────
    # MAE-TinyTransformer'ın Q1'e ulaşmasını sağlayan pretraining adımı.
    # SegFormer3D encoder'ı da 241 hastayla önce self-supervised öğreniyor.
    ...
    # ── Q1 STAGE 2: Fine-tuning (5-Fold Cross-Validation) ────────────────────
    print("=" * 80)
    ...
    # Aggregate OOF (out-of-fold) sonuçları
    ...
    # External test: fold-wise + ensemble (0.5 / Youden / 90%+Sens) + %95 bootstrap CI
