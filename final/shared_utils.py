...
@torch.no_grad()
def evaluate_model(model, loader, criterion, device, use_tta=False):
    """
    Model değerlendirmesi.
    use_tta=True → 4 yönlü flip ortalaması (TTA)
    use_tta=False → Sadece orijinal yönde (temiz, tekrarlanabilir)
...
# istatistiksel olarak karşılaştırmak için 
...
# ============================================================
# ORTAK EXTERNAL TEST DEĞERLENDİRMESİ (fold-wise + ensemble +
...
def evaluate_external_test_ensemble(model_builder, base_dir, config, test_csv_path,
    ...
    for fold_idx in range(1, n_folds + 1):
        ...
        _, _, _, _, pred_df = evaluate_model(model, test_loader, criterion, device, use_tta=False)
    ...
    summary.to_csv(test_dir / "external_test_metrics.csv", index=False)
