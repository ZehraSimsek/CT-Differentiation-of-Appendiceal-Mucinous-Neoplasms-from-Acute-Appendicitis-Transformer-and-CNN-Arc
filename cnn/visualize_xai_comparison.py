...
def render_xai_comparison(
    ...
    for i, res in enumerate(model_results, start=1):
        ...
        if lang == "en":
            pred_str = "Mucinous" if prob >= 0.5 else "Appendicitis"
        else:
            pred_str = "Mukozlu" if prob >= 0.5 else "Apandisit"
        ...
        is_correct = ((prob >= 0.5 and true_label == 1) or (prob < 0.5 and true_label == 0))
...
def main():
    ...
    # Function to robustly find patient H5 file
    def resolve_patient_h5(pid_raw: str, label: int | None = None) -> str | None:
        pid = str(pid_raw).replace(".h5", "").strip()
        candidates = [
            f"datas/Musinoz/musinoz_128/{pid}.h5",
            f"datas/Appendisit/apandisit_128/{pid}.h5",
            f"datas/Musinoz/{pid}.h5",
            f"datas/Appendisit/{pid}.h5",
            f"datas/{pid}.h5",
        ]
        if label == 1:
            candidates.insert(0, f"datas/Musinoz/musinoz_128/{pid}.h5")
        elif label == 0:
            candidates.insert(0, f"datas/Appendisit/apandisit_128/{pid}.h5")

        for c in candidates:
            if os.path.exists(c):
                return c
        
        matches = glob.glob(f"datas/**/{pid}.h5", recursive=True)
        if matches:
            return matches[0]
        return None
